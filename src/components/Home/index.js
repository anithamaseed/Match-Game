...
class Home extends Component {
  ...
  setTimeInterval = () => {
    this.intervalId = setInterval(this.decreaseTimerCount, 1000)
  }
    ...
    return (
      <div className="game-over-bg-container">
        ...
        <p className="score-content">
          YOUR SCORE
          <br />
          <span className="score-view">{score}</span>
        </p>
        <button
          type="button"
  ...
  getRandomIndex = () => {
    ...
    this.setState({activeImageObj: imagesList[randomIndex]})
  }

  onClickImage = id => {
    const {activeImageObj} = this.state
    if (activeImageObj.id === id) {
  ...
  renderGameRunning = () => {
    const {activeImageObj, activeTabId} = this.state
    ...
    return (
      <div className="app-container">
        <div className="display-image">
          <img src={activeImageObj.imageUrl} alt="match" />
        </div>
